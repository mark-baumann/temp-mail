...
export default async function handler(req: Request) {
  ...
  console.log(`[dropmail][${rid}] incoming`, { method: req.method, path: url.pathname, search: url.search, targetUrl });
  ...
  try {
    ...
    console.log(`[dropmail][${rid}] response`, { status: resp.status, ok: resp.ok });
    const headers = new Headers(resp.headers);
    for (const [k, v] of Object.entries(corsHeaders(origin))) headers.set(k, v);
    return new Response(resp.body, { status: resp.status, headers });
  } catch (err: any) {
    console.error(`[dropmail][${rid}] error`, err?.message || err);
